...
beast = xmlrpclib.ServerProxy('http://beast.berkeley.edu:5555', allow_none=True)
...
def show_results():
    ...
    if mode == 1:
        ...
        fs = search_field['fs']
        cat = beast.catalog( (ra,dec), fs, None ) #square box of size fs
        ...
        center, width, offsets = coords_to_offsets( np.array(coords) )
        send_coords = []
        for i in range(len(coords)):
        ...
        center, size = beast.find_field( requested_coords )
        cat = beast.catalog( center, max(size), requested_coords )
        
        # match requested coords to returned sources
        matches,tmp = beast.identify_matches( requested_coords, cat['coords'] )
        ...
        mod_mags = [ sss[FILTER_PARAMS[band][-1]] for sss in cat['SEDs'] ]
        
        # calculate zeropoint
        median_zp, mad_zp, matches, all_zps = beast.calc_zeropoint( requested_coords, cat['coords'], inst_mags, mod_mags )
...
def api_handler():
    ...
    if request.method == 'GET':
        ...
        cat = beast.catalog( (ra,dec), size, None )
